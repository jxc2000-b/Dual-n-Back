...
- 2026-05-11 — Track H landed (feedback + sci-fi polish). Token palette swapped to dark-black / deep-blue / off-white / cyan (names preserved). Grid stim now an ASCII dot matrix via `radial-gradient` — instant on/off, no transitions. HotkeyButtons accept `positionFeedback` / `audioFeedback` props; TrainPage grades each press against `isMatch` at trial-end and flickers green/red for 250 ms. Per user guidance: no animations on the new visuals, instant state changes only.
